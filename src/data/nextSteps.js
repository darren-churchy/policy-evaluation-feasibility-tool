// ============================================================
// Next Steps Registry
// Controls the Recommended Next Steps panel on the Results page.
//
// Each entry specifies:
//   id             — the input field to monitor
//   uncertainValues — answer values that trigger this item
//   page           — display label shown in the results panel
//   question       — short question label
//   guidance       — guidance text shown to the user
//
// Edit guidance text here without touching any component code.
// Add new entries by following the same object structure.
//
// Section labels updated to match new section order (v0.7):
//   Section 2 — Causal Readiness  (was Page 3)
//   Section 4 — Design Questions   (was Page 4)
//   Section 5 — Adjustment & DAG   (was Page 5)
//   Section 6 — Data Sources        (was Page 6)
//   Section 7 — Statistical         (was Page 7)
// ============================================================
...
export const NEXT_STEPS = [
  {
    ...
    page: 'Section 2 — Causal Readiness',
  ...
  },
  {
    ...
    page: 'Section 4 — Design Questions',
  ...
  },
  {
    ...
    guidance: 'Extending the pre-intervention observation period — if data allow — will substantially improve the credibility of the synthetic control. If the pre-period is very short, an augmented synthetic control (ASC) incorporating an outcome model alongside the weighting approach can improve fit and reduce sensitivity to poor pre-period matching. A Bayesian synthetic control may also provide better calibrated uncertainty bounds with short pre-periods.',
  },
  {
    id: 'iv_exclusion',
    uncertainValues: ['maybe', 'no'],
  ...
  },
  {
    ...
    guidance: 'If fewer than 8 pre-intervention time points are available, the ITS trend estimate will be imprecise and assumption testing will be limited. Consider whether older archival data could extend the series, or whether a controlled ITS (adding a comparison series) could compensate for a shorter pre-period. For short series, Bayesian ITS approaches (e.g. CausalImpact) handle limited pre-period data better than classical ARIMA methods by incorporating prior beliefs about the counterfactual trend.',
  },
  {
    id: 'p5_confounder_confidence',
    uncertainValues: ['maybe', 'no'],
    page: 'Section 5 — Adjustment & DAG',
    question: 'Confounder identification confidence',
    guidance: 'Document known unmeasured confounders explicitly in your evaluation protocol. Plan sensitivity analyses — E-values, negative control outcomes, or quantitative bias analysis — to quantify and communicate the potential impact of unmeasured confounding on your conclusions. A Bayesian sensitivity analysis can formally quantify how much unmeasured confounding would be needed to overturn conclusions, which is particularly useful when covariate coverage is incomplete.',
  },
  {
    id: 'p6_linkage',
    uncertainValues: ['maybe', 'no'],
    page: 'Section 6 — Data Sources',
  ...
  },
  {
    ...
    guidance: 'Investigate whether historical administrative data could provide a pre-intervention outcome series. Even limited pre-period data (2–3 time points) substantially improves DiD and ITS credibility compared to no pre-period data, by enabling some assessment of pre-existing trends. For ITS with a short pre-period, Bayesian methods (e.g. CausalImpact) handle limited pre-period data more robustly than classical approaches.',
  },
]
